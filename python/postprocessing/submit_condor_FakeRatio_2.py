...
usage = 'python submit_condor_FakeRatio_2.py -d dataset_name -f destination_folder --trig HT' 
...
def sub_writer(sample, n, files, folder):
    ...
    f.write("transfer_input_files    = $(Proxy_path), samples/samples.py, FakeRatio_utils_2.py, __init__.py\n")
...
for sample in samples:
    ...
    if(isMC):
        for i, files in enumerate(files_list):
            ...
            print("python FakeRatio_2.py " + sample.label + " " + str(i) + " " + str(files) + " remote " + opt.trig)
    else:
        for i in range(len(files_list)/split+1):
            ...
            print("python FakeRatio_2.py " + sample.label + " " + str(i) + " " + ",".join( e for e in files_list[split*i:split*(i+1)]) + " remote " + opt.trig)
