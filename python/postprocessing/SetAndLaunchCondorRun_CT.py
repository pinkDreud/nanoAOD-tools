...
parser.add_option('-t', dest='trig', type=str, default = '', help='Specify which CT runs')
...
optstring = " -f " + folder + " --wp " + str(opt.jetwp + opt.muwp + opt.elewp) + " -t " + str(opt.trig)# + " --wop "
if opt.maxj > 0:
...
for prname, proc in class_dict.items():
    if "Fake" in prname or 'DataMET' in prname:
        continue
    if "DataHT" in prname and opt.trig != 'HT':
        continue
    if ('DataEle' in prname or 'DataEle' in prname) and opt.trig != 'Lep':
        continue
    ...
    if hasattr(proc, 'components'):
        for sample in proc.components:
            ...
            if not AreAllCondored(sample.label):
                if opt.check:
                    ...
                    f.write("python submit_condor_CT.py -d " + sample.label+ " " + optstring)
            else:
        ...
        if not AreAllCondored(proc.label):
            if opt.check:
                ...
                f.write("python submit_condor_CT.py -d " + proc.label+ " " + optstring)
        else:
