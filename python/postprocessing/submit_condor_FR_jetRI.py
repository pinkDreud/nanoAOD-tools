...
def sub_writer(sample, n, files, folder):
    ...
    f.write("transfer_input_files    = $(Proxy_path), samples/samples.py, FakeRatio_utils_jetRI.py, CutsAndValues_bu.py, __init__.py\n")
    ...
    f.write("executable              = FakeRatio_jetRI.py\n")
    f.write("arguments               = " + sample.label + " " + str(n) + " " + str(files) + " remote " + str(opt.trig) + "\n")
    #f.write("input                   = input.txt\n")
    f.write("output                  = condor_FR/output/"+ sample.label + "_part" + str(n) + ".out\n")
    f.write("error                   = condor_FR/error/"+ sample.label +  "_part" + str(n) + ".err\n")
    f.write("log                     = condor_FR/log/"+ sample.label + "_part" + str(n) + ".log\n")
...
if not os.path.exists("condor_FR/output"):
    os.makedirs("condor_FR/output")
if not os.path.exists("condor_FR/error"):
    os.makedirs("condor_FR/error")
if not os.path.exists("condor_FR/log"):
    os.makedirs("condor_FR/log")
...
for sample in samples:
    ...
    if(isMC):
        for i, files in enumerate(files_list):
            ...
            print("python FakeRatio_jetRI.py " + sample.label + " " + str(i) + " " + str(files) + " remote " + str(opt.trig))
    else:
        for i in range(len(files_list)/split+1):
            ...
            print("python FakeRatio_jetRI.py " + sample.label + " " + str(i) + " " + ",".join( e for e in files_list[split*i:extmax]) + " remote " + str(opt.trig))
